...
describe "template_runner" do
  before(:each) do
    setup_template_runner "mocra.rb"
  end
  describe "no authentication" do
    describe "run template for public repo" do
      before(:each) do
        @runner.highline.should_receive(:choose).exactly(1).and_return("none")
  ...
  describe "restful_authentication" do
    describe "run template for private repo" do
      before(:each) do
        @runner.highline.should_receive(:choose).exactly(1).and_return("restful_authentication")
        @runner.on_command(:run, "git config --get github.user") { "github_person\n" }
  ...
  describe "twitter" do
    ...
    describe "run template for read/write twitter registration" do
      before(:each) do
        @runner.highline.should_receive(:choose).exactly(3).
          and_return("twitter_auth", "drnic", "read-write")
    ...
    describe "run template for read-only twitter registration" do
      before(:each) do
        @runner.highline.should_receive(:choose).exactly(3).
          and_return("twitter_auth", "drnic", "read-only")
